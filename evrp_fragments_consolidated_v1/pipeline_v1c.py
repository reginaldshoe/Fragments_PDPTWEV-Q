"""Consolidated-v1c pipeline using split fragment, master and callback layers.

Environment:
- Python 3.12
- gurobipy available in the target environment

Role: draft output v1c / modular split candidate.

Fragment construction is in fragment_core. Master/network/model construction is
in master_core. Callback, DP and route tools are in callback_core. legacy_core
remains present as the generated behavioural source reference for this stage.
"""
...
FRAGMENT_HELPERS_FOR_MASTER: tuple[str, ...] = (
    ...
    "compute_T_E_L", "compute_distance", "compute_Emin", "cust_locs",
)

FRAGMENT_HELPERS_FOR_CALLBACK: tuple[str, ...] = (
    "is_station", "is_customer", "is_pickup", "is_delivery",
    "compute_T_E_L", "compute_distance", "compute_Emin", "strip_stations",
)


@dataclass(frozen=True)
class ConsolidatedV1cRunSummary:
    package: str
...
def _inject_fragment_helpers_into_callback() -> None:
    for name in FRAGMENT_HELPERS_FOR_CALLBACK:
        if hasattr(frag, name):
            setattr(cbcore, name, getattr(frag, name))
...
def _sync_all_globals() -> None:
    ...
    _inject_fragment_helpers_into_callback()
    _sync_module_globals(cbcore)
...
def run_solver(instance: str | Path, max_base_path_len: int, k_max: int, force_exact_k: bool, use_callback: bool) -> ConsolidatedV1cRunSummary:
    ...
    cbcore.data = data
    cbcore.e_frags_aug = ef
    cbcore.arc_by_id = arc_by_id
    cbcore.arcs_full = arcs
    cbcore.node_id_tmp = node_id
    cbcore.depot_u_tmp = depot_u
    cbcore.theta_tmp = theta
    cbcore.M_tmp = big_m
    cbcore.x_tmp = x_vars
    _sync_all_globals()
    ...
    return ConsolidatedV1cRunSummary(
        package="evrp_fragments_consolidated_v1c_callback_split",
...
def summary_to_json(summary: ConsolidatedV1cRunSummary) -> str:
