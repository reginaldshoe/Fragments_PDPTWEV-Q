"""Resource calculations used by fragments and the master model."""
...
def max_dist(data: dict[str, Any]) -> float:
    ...
    for i in range(len(data['nodes'])):
        for j in range(len(data['nodes'])):
            if i == j:
                continue
            max_distance = max(max_distance, data['dist'](i, j))
    return max_distance
...
def compute_T_E_L(data: dict[str, Any], seq: tuple[str, ...] | list[str]) -> tuple[float, float, float]:
    ...
    for u, v in zip(seq, seq[1:]):
        tf += process_time(u) + data['traveltime'](sid_to_i[u], sid_to_i[v])
    t = ready(seq[0])
    for u, v in zip(seq, seq[1:]):
        t = t + process_time(u) + data['traveltime'](sid_to_i[u], sid_to_i[v])
        t = max(ready(v), t)
    ef = t
    t = due(seq[-1])
    for u, v in zip(reversed(seq[:-1]), reversed(seq[1:])):
        t = t - data['traveltime'](sid_to_i[u], sid_to_i[v]) - process_time(u)
        t = min(due(u), t)
    lf = t
    return tf, ef, lf
