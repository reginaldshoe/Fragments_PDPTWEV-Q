"""Consolidated-v1d pipeline with centralised legacy global bindings.

Environment:
- Python 3.12
- gurobipy available in the target environment

Role: draft output v1d / cleanup globals candidate.

This stage does not alter algorithm bodies. It moves cross-module binding logic
out of the solver runner and into dependency_bridge.py so remaining legacy
module-global assumptions are explicit and auditable.
"""
...
class ConsolidatedV1dRunSummary:
    package: str
...
def build_fragment_sets(instance: str | Path, max_base_path_len: int) -> dict[str, Any]:
    _ensure_generated()
    deps.bind_all_static_dependencies(frag, master, cbcore, core)

    data = frag.read_instance(Path(instance))
    deps.bind_data_context(data, frag, master, cbcore, core)
...
def run_solver(instance: str | Path, max_base_path_len: int, k_max: int, force_exact_k: bool, use_callback: bool) -> ConsolidatedV1dRunSummary:
    sets = build_fragment_sets(instance, max_base_path_len)
    data = sets["data"]
    ef = sets["extended_undominated"]

    deps.bind_fragment_context(ef, master, cbcore, core)
    deps.bind_all_static_dependencies(frag, master, cbcore, core)
    ...
    return ConsolidatedV1dRunSummary(
        package="evrp_fragments_consolidated_v1d_cleanup_globals",
...
def summary_to_json(summary: ConsolidatedV1dRunSummary) -> str:
