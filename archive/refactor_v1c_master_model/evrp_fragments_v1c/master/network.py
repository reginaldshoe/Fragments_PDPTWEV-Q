...
from ..types import NetworkBuildResult
...
def build_network(fragment_arcs: list[dict[str, Any]]) -> NetworkBuildResult:
    ...
    for idx, frag in enumerate(fragment_arcs):
        start_onboard = frozenset(frag['start_onboard'])
        end_onboard = frozenset(frag['end_onboard'])
        u = get_node(frag['Start'], start_onboard)
        v = get_node(frag['End'], end_onboard)
        arcs.append({
            'id': idx,
            'u': u,
            'v': v,
            ...
            'Lf': float(frag['Lf']),
            'Df': float(frag['Df']),
            'Emin': float(frag.get('Emin', frag.get('min_start_energy', 0.0))),
            'LocsC': frag.get('LocsC', frozenset()),
        })
    return NetworkBuildResult(node_id=node_id, arcs=arcs)
